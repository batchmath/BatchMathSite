...
function radHTML(a){let [n,d]=a.rad;if(n===0)return '0';if(d===1)return n===1?'π':`${n}π`;const top=n===1?'π':`${n}π`;return `<span class="math-frac"><span class="num">${top}</span><span class="bar"></span><span class="den">${d}</span></span>`}
function radHTMLdeg(deg){if(deg===0)return '0';const g=B.gcd(Math.abs(deg),180),n=deg/g,d=180/g;if(d===1)return n===1?'π':n===-1?'−π':`${n}π`;const sign=n<0?'−':'',an=Math.abs(n),top=an===1?'π':`${an}π`;return `${sign}<span class="math-frac"><span class="num">${top}</span><span class="bar"></span><span class="den">${d}</span></span>`}
function exactHTML(s){if(s==='undefined')return 'Undefined';let neg=s.startsWith('-');if(neg)s=s.slice(1);const idx=s.indexOf('/');let h;if(idx>0){h=`<span class="math-frac"><span class="num">${B.inlineMathHTML(s.slice(0,idx))}</span><span class="bar"></span><span class="den">${B.inlineMathHTML(s.slice(idx+1))}</span></span>`}else h=B.inlineMathHTML(s);return (neg?'−':'')+h}
...
function explanation(c){const a=c.a,f=c.f,angle=c.r?radHTML(a):`${a.deg}°`;if(c.canonical==='undefined'){return `${f} is undefined at ${angle} because the reciprocal/quotient definition would require division by 0.`}if([0,90,180,270].includes(a.deg)){return `Use the unit-circle point (cos θ, sin θ) at ${angle}. The exact value is ${exactHTML(c.canonical)}.`}const qn=['','I','II','III','IV'][quadrant(a.deg)],ref=c.r?radHTMLdeg(a.ref):`${a.ref}°`;return `The reference angle is ${ref}. Start with the first-quadrant ${f} value ${exactHTML(base[a.ref][f])}, then apply the sign of ${f} in Quadrant ${qn}.`}
const editor=BMExactEditor.mount({input:'#answerRaw',editor:'#exactEditor',keypad:'#exactKeypad',onCheck:check});
function newQ(){locked=false;editor.lock(false);editor.clear();$('feedback').innerHTML='';$('feedback').className='feedback';next.style.display='none';let c,tries=0;do{c=make();tries++;if(tries>80){seen.clear();break}}while(seen.has(c.id));seen.add(c.id);current=c;prompt.textContent='Find the exact value.';B.renderHTML(q,`${names[c.f]}(${c.r?radHTML(c.a):`${c.a.deg}°`})`);editor.focus()}
function check(){if(locked)return;const raw=editor.raw.trim();if(!raw)return;attempted++;let ok=false;if(current.canonical==='undefined'){ok=/^(?:undefined|undef|dne)$/i.test(raw.trim())}else if(!raw.includes('.')){const v=B.safeExactEval(raw);ok=Number.isFinite(v)&&Math.abs(v-current.expected)<1e-8}B.updateStats(correct+(ok?1:0),attempted);if(ok){correct++;B.updateStats(correct,attempted);B.showFeedback(true);locked=true;editor.lock(true);setTimeout(newQ,600)}else{B.showFeedback(false,exactHTML(current.canonical),explanation(current));locked=true;editor.lock(true);next.style.display='inline-block'}}
