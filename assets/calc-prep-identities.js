...
function make(){let typ=qtype.value;if(typ==='mixed')typ=BatchMathRNG.random()<.55?'recall':'rewrite';if(typ==='recall'){const p=poolFor(F),f=B.pick(p),valid=[...new Set(p.filter(x=>x.lhs===f.lhs).map(x=>x.rhs))],ds=distract(f.rhs,p,valid);return {id:'rec-'+f.id,prompt:'Complete the identity.',question:`${f.lhs} = ?`,answer:f.rhs,validAnswers:valid,choices:[f.rhs,...ds],method:`Identity: ${f.lhs} = ${f.rhs}.`}}const p=poolFor(R),r=B.pick(p),ds=distract(r.ans,p);return {id:'rew-'+r.id,prompt:'Choose an equivalent form.',question:r.expr,answer:r.ans,validAnswers:[r.ans],choices:[r.ans,...ds],method:`Using the relevant trigonometric identity, ${r.expr} rewrites as ${r.ans}.`}}
