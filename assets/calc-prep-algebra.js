...
function newQ(){locked=false;choicesEl.dataset.locked='0';$('feedback').innerHTML='';$('feedback').className='feedback';next.style.display='none';freeAnswer.value='';current=make();prompt.textContent=current.prompt;B.renderHTML(question,current.questionHTML);if(current.kind==='choice'){choicesEl.style.display='grid';freeArea.style.display='none';getChoice=B.choiceButtons(choicesEl,current.choices.map(x=>({value:x,text:x})))}else{choicesEl.style.display='none';freeArea.style.display='block';getChoice=()=>null;setTimeout(()=>freeAnswer.focus(),0)}}
