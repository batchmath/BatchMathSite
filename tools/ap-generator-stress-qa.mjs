...
function elem(value=''){return {dataset:{},value,selectedOptions:[{textContent:value}],innerHTML:'',textContent:'',className:'',disabled:false,placeholder:'',style:{display:''},listeners:{},addEventListener(type,fn){this.listeners[type]=fn;},focus(){},classList:{add(){},remove(){},contains(){return false;},toggle(){}}};}
...
for(const spec of specs){
  ...
  for(const category of spec.categories){
    ...
    vm.createContext(sandbox);sandbox.window.BatchMathRNG=sandbox.BatchMathRNG;for(const dep of ['ap-topic-generators','unit1-course-trig','derivative-rule-factors'])vm.runInContext(fs.readFileSync(path.join(ROOT,'assets/'+dep+'.js'),'utf8'),sandbox);sandbox.BatchMathDerivativeRuleFactors=sandbox.window.BatchMathDerivativeRuleFactors;
    try{
