...
function sampleContinuousMerged(seedBase){
  const box={window:{BatchMathRNG:{random:rng(seedBase)}},console};box.BatchMathRNG=box.window.BatchMathRNG;vm.createContext(box);
  vm.runInContext(fs.readFileSync(path.join(ROOT,'assets/unit1-core-expansions.js'),'utf8'),box,{filename:'unit1-core-expansions.js'});
  const out=[];for(let i=0;i<PER_TOPIC;i++){box.window.BatchMathRNG.random=rng(`${seedBase}:${i}`);out.push(structuredClone(box.window.BMUnit1CoreExpansions.continuous('mixed')));}return out;
}
function sampleFiltered(html,category,seedBase){
  ...
  const sb={window,document:fakeDocument,BatchMathRNG:{random},console,Math,structuredClone,setTimeout:()=>0,clearTimeout:()=>{},location:{},navigator:{}};vm.createContext(sb);sb.window.BatchMathRNG=sb.BatchMathRNG;for(const dep of ['ap-topic-generators','unit1-course-trig','unit1-core-expansions','unit1-basic-techniques','unit1-one-sided','unit1-infinity-legacy','unit1-infinity-expansions','derivative-rule-factors'])vm.runInContext(fs.readFileSync(path.join(ROOT,'assets/'+dep+'.js'),'utf8'),sb);sb.BatchMathDerivativeRuleFactors=sb.window.BatchMathDerivativeRuleFactors;vm.runInContext(code,sb,{timeout:8000});if(listeners.load)listeners.load();
...
function sampleSpecial(file,id,seedBase){
  ...
  if(id==='ap_topic_derivatives_with_the_power_rule'){
    const html=fs.readFileSync(file,'utf8'),code=inlineScripts(html).find(x=>x.includes('function powerRuleProblem')&&x.includes('function parseExpression'));if(!code)throw Error('Power Rule family script missing');const close=code.lastIndexOf('})();'),body=code.slice(0,close)+'\n;globalThis.QA={powerRuleProblem};\n'+code.slice(close),elements={};const elemFor=id=>elements[id]||(elements[id]=elem());const sb={console,Math,BatchMathRNG:{random:rng(seedBase)},window:{MathJax:null,BatchMathCalculusKeypad:null,BMAnalytics:null,addEventListener(){}},document:{getElementById:elemFor,addEventListener(){},querySelector(){return null},querySelectorAll(){return[]}},setTimeout:()=>0,clearTimeout:()=>{},location:{},navigator:{}};vm.createContext(sb);vm.runInContext(body,sb,{filename:path.basename(file),timeout:8000});const out=[];for(let i=0;i<PER_TOPIC;i++){sb.BatchMathRNG.random=rng(`${seedBase}:${i}`);const p=structuredClone(sb.QA.powerRuleProblem(i%4===0?'higher':'first','mixed'));p.variant=(p.id.match(/^pr-([^-]+)-(\d)/)||['','power',''])[1]+'-'+((p.id.match(/^pr-[^-]+-(\d)/)||[])[1]||'');out.push(p)}return out;
  }
...
for(const file of topicFiles){
  ...
  try{
    ...
    else if(id==='ap_topic_limits_of_continuous_functions'){sourceType='shared_unit1_module';sourceKey='continuous:mixed';problems=sampleContinuousMerged(`deep:${sourceKey}`);}
    else if(['ap_limit_proofs','ap_classifying_discontinuities','ap_advanced_trig_limits','ap_topic_derivatives_with_the_power_rule'].includes(id)){sourceType='specialized';sourceKey=id;problems=sampleSpecial(file,id,`deep:${id}`);}
...
const totals={topics:records.length,shared:records.filter(r=>r.sourceType==='shared_topic_generator').length,modular:records.filter(r=>r.sourceType==='shared_unit1_module').length,filtered:records.filter(r=>r.sourceType==='filtered_comprehensive').length,specialized:records.filter(r=>r.sourceType==='specialized').length,samples:records.reduce((s,r)=>s+r.samples,0)};
...
const txt=['BatchMath AP individual-topic deep audit',`Result: ${report.ok?'PASS':'FAIL'}`,`Topics: ${totals.topics} (${totals.shared} shared topic, ${totals.modular} shared module, ${totals.filtered} filtered comprehensive, ${totals.specialized} specialized)`,`Raw generated samples: ${totals.samples}`,`Samples per topic: ${PER_TOPIC}`,'',`Warnings: ${warnings.length}`,...warnings.map(x=>`- ${x}`),'',`Errors: ${errors.length}`,...errors.slice(0,200).map(x=>`- ${x}`),''];fs.writeFileSync(path.join(OUT,'ap-topic-deep-audit.txt'),txt.join('\n'));
const md=['# AP Calculus Individual Practice - Generator Coverage','','Generated from the actual current site generators. Raw repeat rates are measured before the six-problem recent-ID suppression used by shared AP topic pages.','',`**Coverage:** ${totals.topics} topic pages - ${totals.shared} shared topic generators, ${totals.modular} shared Unit 1 module, ${totals.filtered} filtered comprehensive generators, ${totals.specialized} specialized generators.`,`**Audit sample:** ${PER_TOPIC.toLocaleString()} raw generations per topic (${totals.samples.toLocaleString()} total).`,'', '| Unit | Topic | Source | Families / variants observed | Unique IDs in sample | Raw immediate repeat |','|---|---|---|---|---:|---:|'];
