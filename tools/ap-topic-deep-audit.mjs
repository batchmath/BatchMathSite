...
function sampleFiltered(html,category,seedBase){
  ...
  const sb={window,document:fakeDocument,BatchMathRNG:{random},console,Math,structuredClone,setTimeout:()=>0,clearTimeout:()=>{},location:{},navigator:{}};vm.createContext(sb);sb.window.BatchMathRNG=sb.BatchMathRNG;for(const dep of ['ap-topic-generators','unit1-course-trig','unit1-core-expansions','unit1-one-sided','derivative-rule-factors'])vm.runInContext(fs.readFileSync(path.join(ROOT,'assets/'+dep+'.js'),'utf8'),sb);sb.BatchMathDerivativeRuleFactors=sb.window.BatchMathDerivativeRuleFactors;vm.runInContext(code,sb,{timeout:8000});if(listeners.load)listeners.load();
