...
const els=new Map();function el(id){if(!els.has(id))els.set(id,{id,style:{},dataset:{},hidden:false,disabled:false,value:'',innerHTML:'',textContent:'',className:'',events:{},addEventListener(t,fn){this.events[t]=fn},setAttribute(k,v){this[k]=v},focus(){},setSelectionRange(){},click(){this.events.click?.()}});return els.get(id);}
let generated=0,checked=0,lastProblem;const sb={BatchMathRNG:{random},window:{BMAnalytics:{ensurePracticeStarted(){},problemGenerated(p){generated++;lastProblem=p;},answerChecked(){checked++},solutionRevealed(){}}},document:{getElementById:el,querySelectorAll:()=>[]},Promise};vm.runInNewContext(code,sb);
