...
assert(page.includes('<header class="topbar">')&&page.includes('<a class="logo" href="/">BatchMath<span class="tm-mark">™</span></a>'),'advanced trig must use the standard one-color site header');
assert(page.includes('<h1 class="page-title">Advanced Trigonometric Limits Practice</h1>')&&!page.includes('<span class="math">MATH</span>'),'advanced trig title/header style does not match the site');
assert(/<div class="controls"><div class="control"><label for="focus" class="bm-practice-select-label">Practice Type<\/label><select id="focus"[^>]*class="bm-practice-select"/.test(page)&&!page.includes('class="focus-label"'),'advanced trig focus selector must use the shared practice-select treatment');
...
quickKeys.find(b=>b.dataset.action==='backspace').click();assert.equal(el('answer').value,'');
quickKeys.find(b=>b.dataset.action==='sqrt').click();quickKeys.find(b=>b.dataset.action==='sqrt').click();
assert.equal(el('answer').value,'√(√())');quickKeys.find(b=>b.dataset.action==='backspace').click();assert.equal(el('answer').value,'√()');
quickKeys.find(b=>b.dataset.action==='backspace').click();assert.equal(el('answer').value,'');
quickKeys.find(b=>b.dataset.action==='sqrt').click();el('answer').setSelectionRange(0,0);el('answer').events.keydown({key:'Delete',preventDefault(){}});assert.equal(el('answer').value,'');
quickKeys.find(b=>b.dataset.action==='sqrt').click();
