...
for(let i=0;i<20000;i++){
 const p=c.window.BMUnit1CourseTrig.generate();trigSamples++;trigFamilies[p.variant]=(trigFamilies[p.variant]||0)+1;assert.equal(p.answerType,'numeric');assert(!p.choices);assert(Number.isFinite(p.numericAnswer));assert(p.explanation.length>100);assert(!/NaN|undefined/.test(p.questionHtml));
	 if(p.courseData&&p.questionHtml.includes('\\lim_{x\\to0}')){const expr=p.questionHtml.split('\\lim_{x\\to0}')[1].split('\\)')[0],h=p.variant==='expanded_cosine_fourth'?.03:.000001;const near=[-h,h].map(x=>c.evaluate(expr,{x}));const expected=p.numericAnswer;for(const v of near)assert(Math.abs(v-expected)<.02*Math.max(1,Math.abs(expected)),p.variant+': '+v+' vs '+expected);}
...
assert.equal(Object.keys(discFamilies).length,13);assert(factored/eligible>.04&&factored/eligible<.06,'factored share');
const discPage=read('ap-calculus/unit-1-limits-continuity/topics/introduction-to-continuity/practice/index.html');assert(discPage.includes('/assets/unit1-discontinuities.js'));assert(discPage.includes('window.BMUnit1Discontinuities?.families'));
const review=read('ap-calculus/unit-1-limits-continuity/topics/comprehensive-review/practice/index.html'),reviewController=read('assets/unit1-comprehensive-review.js');assert(!review.includes('value="asymptotes"'));assert(!review.includes(",'asymptotes'"));assert(review.includes('/assets/unit1-comprehensive-review.js'));assert(reviewController.includes('BMUnit1CourseTrig.generate()'));assert(review.includes('<h1 class="page-title">Comprehensive Review</h1>'));
